...
void timsort(T &array, int size) {
    ...
    do {
        ...
        if (next != size){
            if (array[next] - array[cur] >= 0) {
                ...
                while (next < size && (array[next] - array[next - 1]) < 0)
                    next++;
                // Reversing
                for (int i = 0; i < (int)((next - cur) / 2); i++){
                    int tmp = array[cur + i];
                    array[cur + i] = array[next - i - 1];
                    array[next - i - 1] = tmp;
                }
            }
            run = next - cur;
        ...
        if (run < min_run) {
            ...
            ins_sort(array, cur, cur + run_extended, cur + run);
            run = run_extended;
        }
        merge_queue.push_back(std::make_pair(cur, run));
        // Merging
        while (merge_queue.size() > 1) {
            int n = merge_queue.size() - 2;
            if ((n > 0 && merge_queue[n - 1].second <= merge_queue[n].second + merge_queue[n + 1].second) ||
                (n > 1 && merge_queue[n - 2].second <= merge_queue[n - 1].second + merge_queue[n].second)) {
                if (merge_queue[n - 1].second <= merge_queue[n + 1].second) {
                    --n;
                }
                merge(array, merge_queue[n].first, merge_queue[n + 1].first, merge_queue[n + 1].first + merge_queue[n + 1].second);
                merge_queue[n].second += merge_queue[n + 1].second;
                if (n == merge_queue.size() - 3){
                    merge_queue[n + 1] = merge_queue[n + 2];
                }
                merge_queue.pop_back();
            } else if (merge_queue[n].second <= merge_queue[n + 1].second) {
                merge(array, merge_queue[n].first, merge_queue[n + 1].first, merge_queue[n + 1].first + merge_queue[n + 1].second);
                merge_queue[n].second += merge_queue[n + 1].second;
                merge_queue.pop_back();
            } else {
    ...
    while (merge_queue.size() > 1) {
            int n = merge_queue.size() - 2;
            if (n > 0 && merge_queue[n - 1].second < merge_queue[n + 1].second) {
                --n;
            }
            merge(array, merge_queue[n].first, merge_queue[n + 1].first, merge_queue[n + 1].first + merge_queue[n + 1].second);
            merge_queue[n].second += merge_queue[n + 1].second;
